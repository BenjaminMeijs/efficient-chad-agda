...
chad-equiv-DSemᵀ : {Γ : Env Pr} {τ : Typ Pr} 
                ...
                → (∃-dsyn : FooBar (Etup-to-val a) t)
                → let dsem = FooBar→DSem a t ∃-dsyn
...
chad-equiv-DSemᵀ {Γ} a evIn (just ctg) (pair {σ = σ} {τ = τ} l r) ~τ ~Γ (foobar dsyn)
  = let ctgL = _ ; ctgR = _
        ...
        -- (dl , dr) = Pair.DSemᵀ-exists-lemma-pair₁ (interp l ∘ Etup-to-val) (interp r ∘ Etup-to-val) a ?
        ihr = chad-equiv-DSemᵀ a (LACMexec m1 evIn) ctgR r (~τ .snd) (chad-preserves-≃Γ (Etup-to-val a) evIn ctgL l (~τ .fst) ~Γ) (foobar (dsyn .snd))
        ihl = chad-equiv-DSemᵀ a evIn ctgL l (~τ .fst) ~Γ (foobar (dsyn .fst))
  in begin
...
... | inj₁ x | foobar dsyn
  ...
  -- = let -- DSemᵀ of subterms is defined
          ...
          apply-eq-to-dsyn : [ (λ v' → DSyn-Exists (push v' (Etup-to-val a)) l) , (λ v' → DSyn-Exists (push v' (Etup-to-val a)) r) ] (interp e $ Etup-to-val a) ≡ DSyn-Exists (push x (Etup-to-val a)) l
          apply-eq-to-dsyn rewrite interp-e-val≡inj-x = refl
          dsyn' : DSyn-Exists (push x (Etup-to-val a)) l
          ...
          ih-e = chad-equiv-DSemᵀ a (body .snd) (just ∘ inj₁ $ body .fst) e body-preserves≃τ (snd body-preserves≃Γ) (foobar (dsyn .fst))
          ih-l = trans (chad-equiv-DSemᵀ (x , a) ev' ctg l ~τ ev'≃ev (foobar dsyn')) 
...
--         -- ih-l = trans (chad-equiv-DSemᵀ (x , a) ev' ctg l ~τ ev'≃ev ?) 
--         --              (cong₂ _,_ (plusvDense-zeroR' {{zerov-equiv-zerovDense (D2τ' σ)}}) refl)
--         ih = {!   !} -- trans ih-e (cong₂ _ev+_ (gnoc (cong fst ih-l) λ q → Etup2EV (to-witness de (q , zerovDense (D2τ' τ)))) (cong snd ih-l))
--     in begin
--     LEtup2EV (LACMexec (interp (chad e) (Etup-to-val-primal a) .snd (just (inj₁ (body .fst))) .fst) (body .snd))
...
-- ... | inj₂ x = {!   !}
  ...
  --   Etup2EV (to-witness d-case (sparse2dense ctg)) ev+ LEtup2EV evIn
  --   ∎
chad-equiv-DSemᵀ {Γ} a evIn ctg t ~τ ~Γ dsyn = {!   !}
-- -- Cases where ctg is (semantically) zero
-- chad-equiv-DSemᵀ {Γ} a evIn tt unit ~τ ~Γ dsyn 
--   rewrite chad-ctg-zero (Etup-to-val a) evIn tt unit tt ~Γ refl
--   = Ev-zero.DSemᵀ-ev-lemma-ctg-zero-evIn' (DSyn→DSem a unit tt)
-- chad-equiv-DSemᵀ {Γ} a evIn nothing (pair {σ = σ} {τ = τ} l r) ~τ ~Γ dsyn
--   rewrite chad-ctg-zero (Etup-to-val a) evIn nothing (pair l r) tt ~Γ refl 
--   = Ev-zero.DSemᵀ-ev-lemma-ctg-zero-evIn' (DSyn→DSem a (pair l r) dsyn)
-- chad-equiv-DSemᵀ {Γ} a evIn nothing (inl {σ = σ} {τ = τ} t) ~τ ~Γ dsyn
--   rewrite chad-ctg-zero (Etup-to-val a) evIn nothing (inl {σ = σ} {τ = τ} t) tt ~Γ refl 
--   = Ev-zero.DSemᵀ-ev-lemma-ctg-zero-evIn' (DSyn→DSem a (inl t) dsyn)
-- chad-equiv-DSemᵀ {Γ} a evIn nothing (inr {σ = σ} {τ = τ} t) ~τ ~Γ dsyn
--   rewrite chad-ctg-zero (Etup-to-val a) evIn nothing (inr {σ = σ} {τ = τ} t) tt ~Γ refl 
--   = Ev-zero.DSemᵀ-ev-lemma-ctg-zero-evIn' (DSyn→DSem a (inr t) dsyn)
-- -- Cases where ctg is NOT (semantically) zero
-- chad-equiv-DSemᵀ {Γ} a evIn ctg (var idx) ~τ ~Γ dsyn 
...
--   ∎
-- chad-equiv-DSemᵀ {Γ} a evIn (just ctg) (pair {σ = σ} {τ = τ} l r) ~τ ~Γ dsyn
--   = let ctgL = _ ; ctgR = _
--         m1 = interp (chad l) (Etup-to-val-primal a) .snd ctgL .fst
--         m2 = interp (chad r) (Etup-to-val-primal a) .snd ctgR .fst
...
--         dr = DSyn→DSem a r (dsyn .snd)
--         -- (dl , dr) = Pair.DSemᵀ-exists-lemma-pair₁ (interp l ∘ Etup-to-val) (interp r ∘ Etup-to-val) a ?
--         ihr = chad-equiv-DSemᵀ a (LACMexec m1 evIn) ctgR r (~τ .snd) (chad-preserves-≃Γ (Etup-to-val a) evIn ctgL l (~τ .fst) ~Γ) (dsyn .snd)
--         ihl = chad-equiv-DSemᵀ a evIn ctgL l (~τ .fst) ~Γ (dsyn .fst)
--   in begin
--   LEtup2EV (LACMexec (LACMsequence m1 m2) evIn)
--     ≡⟨ gnoc (LACMexec-sequence m1 m2 evIn) LEtup2EV ⟩
--   LEtup2EV (LACMexec m2 (LACMexec m1 evIn))
...
--   (Etup2EV (to-witness dt (sparse2dense {D2τ' σ :*! D2τ' τ} (just ctg))) ev+ LEtup2EV evIn)
--   ∎
-- chad-equiv-DSemᵀ {Γ} a evIn ctg (fst' {σ = σ} {τ = τ} t) ~τ ~Γ dsyn
...
--     ≡⟨ chad-equiv-DSemᵀ a evIn ctg' t (~τ , ≃τ-zerov' τ) ~Γ dsyn ⟩
...
--   ∎
-- chad-equiv-DSemᵀ {Γ} a evIn ctg (snd' {σ = σ} {τ = τ} t) ~τ ~Γ dsyn = {!   !}
-- chad-equiv-DSemᵀ {Γ} a evIn ctg (let' {σ = σ} {τ = τ} rhs body) ~τ ~Γ dsyn =
...
--       ih-body = trans (chad-equiv-DSemᵀ a' (zerov (D2τ' σ) .fst , evIn) ctg body ~τ (≃Γ-intro-zero' σ evIn ~Γ) (dsyn .snd))
--                       (cong₂ _,_ (plusvDense-zeroR' {{zerov-equiv-zerovDense (D2τ' σ) }}) refl)
--       ih-rhs = chad-equiv-DSemᵀ a (ev-body .snd) (ev-body .fst) rhs (fst preserves-≃Γ) (snd preserves-≃Γ) (dsyn .fst)
...
--   Etup2EV (to-witness dt (sparse2dense ctg)) ev+ LEtup2EV evIn
--   ∎
-- chad-equiv-DSemᵀ {Γ} a evIn ctg (prim {σ = σ} {τ = τ} op t) ~τ ~Γ dsyn =
...
--     ≡⟨ chad-equiv-DSemᵀ a evIn d-chad-op t (dprim'-preserves-≃τ (Etup-to-val a) ctg op t ~τ) ~Γ (dsyn .snd) ⟩
...
--   ∎
-- chad-equiv-DSemᵀ {Γ} a evIn (just (inj₁ ctg)) (inl {σ = σ} {τ = τ} t) ~τ ~Γ dsyn =
...
--     ≡⟨ chad-equiv-DSemᵀ a evIn ctg t ~τ ~Γ dsyn ⟩
--   Etup2EV (to-witness dt (sparse2dense ctg)) ev+ LEtup2EV evIn
--     ≡⟨ ev+congL (gnoc (DSemᵀ-lemma-inj₁ _ a dt d-inl _ _) Etup2EV) ⟩
--   Etup2EV (to-witness d-inl ctg') ev+ LEtup2EV evIn
--   ∎
-- chad-equiv-DSemᵀ {Γ} a evIn (just (inj₂ ctg)) (inr {σ = σ} {τ = τ} t) ~τ ~Γ dsyn = {!   !}
-- chad-equiv-DSemᵀ {Γ} a evIn ctg (case' {σ = σ} {τ = τ} {ρ = ρ}  e l r) ~τ ~Γ dsyn = ?
  -- rewrite chad-preserves-primal (Etup-to-val a) e
  -- = case (interp e $ Etup-to-val a) of [ caseL , caseR ]
  -- where dl : Is-just (DSemᵀ {Etup Pr Γ} {ρ} (interp (case' e l r) ∘ Etup-to-val) a)
  --       dl = DSyn→DSem a (case' e l r) dsyn
  --       caseL : (x : Rep σ) → LEtup2EV (LACMexec (interp (chad (case' e l r)) (Etup-to-val-primal a) .snd ctg .fst) evIn) ≡ (Etup2EV (to-witness dl (sparse2dense ctg)) ev+ LEtup2EV evIn)
  --       caseL = {!   !}
  --       -- caseL x with interp e (Etup-to-val a) 
  --       -- ... | w = {!   !}
  --       caseR : (x : Rep τ) → LEtup2EV (LACMexec (interp (chad (case' e l r)) (Etup-to-val-primal a) .snd ctg .fst) evIn) ≡ (Etup2EV (to-witness dl (sparse2dense ctg)) ev+ LEtup2EV evIn)
  --       caseR x = {! simplify-exec-chad-case  !}
  -- with interp e (Etup-to-val a) in interp-e-val≡inj-x
  -- with dsyn
-- ... | w = ?
