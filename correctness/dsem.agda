...
postulate
    ...
    DSemᵀ-var : {Γ : Env Pr} {τ : Typ Pr}
        ...
        → Σ (Is-just $ DSemᵀ {ET Pr Γ} {τ} (flip valprj idx ∘ ET-to-val) a)
...
--     -- ======================
    DSemᵀ-prim-floatPlus :
          (a : Rep {Pr} (R :* R))
        → (ctg : LinRepDense (D2τ' R))
        → let (x , y) = a
        in Σ ( Is-just $ DSemᵀ {R :* R} {R} (uncurry primFloatPlus) a)
             ( λ df → to-witness df ctg ≡ (ctg , ctg) )

    DSemᵀ-prim-floatTimes :
          (a : Rep {Pr} (R :* R))
        → (ctg : LinRepDense (D2τ' R))
        → let (x , y) = a
        in Σ ( Is-just $ DSemᵀ {R :* R} {R} (uncurry primFloatTimes) a)
                ( λ df → to-witness df ctg
                         ≡ (primFloatTimes ctg y , primFloatTimes ctg x))

    DSemᵀ-prim-floatNegate : 
          (a : Rep {Pr} R) 
        → (ctg : LinRepDense (D2τ' R))
        → Σ (Is-just $ DSemᵀ {R} {R} primFloatNegate a)
