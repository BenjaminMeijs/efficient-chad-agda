...
-- =======================================
-- MOTIVATION:
-- For chad-equiv-dsem, when the term is (case' e l r), we wish to perform a with-abstraction on 'interp e (ET-to-val a)'.
-- If chad-equiv-dsem has DSyn-ExistsP in its goal, then this with-abstraction also impacts this.
-- This leads to an ill-typed abstraction, as 'interp e (ET-to-val a)' is part of the definition of DSyn-ExistsP.
...
-- Then, after having done a with-abstraction on 'interp e (ET-to-val a)', we can with-abstract on DSyn-Exists to obtain the underlying predicate.
...
module DSyn-defined-implies-DSem-defined where
    ...
    private module case-helper {Γ : Env Pr} {σ τ ρ : Typ Pr} (a : Rep (ET Pr Γ)) (e : Term Pr Γ (σ :+ τ)) (l : Term Pr (σ ∷ Γ) ρ) (r : Term Pr (τ ∷ Γ) ρ) where
        f : (Rep $ (σ :+ τ) :* ET Pr Γ) → Rep ρ
        f = λ (cond , a') → [ (λ v → interp l $ ET-to-val (v , a'))
                            , (λ v → interp r $ ET-to-val (v , a'))
                            ] cond
        g : (a' : Rep (ET Pr Γ)) → Rep ((σ :+ τ) :* ET Pr Γ)
        g = λ a' → interp e (ET-to-val a') , a'

        case-simp-ext : (f ∘ g) ≗ interp (case' e l r) ∘ ET-to-val
    ...
    ∃DSyn→∃DSem {Γ} {τ} a ( fst' t ) w = DSemᵀ-exists-lemma-chain fst (interp t ∘ ET-to-val) a (DSemᵀ-fst (interp t $ ET-to-val a) (zerovDense (D2τ' τ))  .fst) (∃DSyn→∃DSem a t w)
    ∃DSyn→∃DSem {Γ} {τ} a ( snd' t ) w = DSemᵀ-exists-lemma-chain snd (interp t ∘ ET-to-val) a (DSemᵀ-snd (interp t $ ET-to-val a) (zerovDense (D2τ' τ))  .fst) (∃DSyn→∃DSem a t w)
    ∃DSyn→∃DSem {Γ} {τ} a ( let' {σ = σ} rhs body ) w =
      let ih-rhs = DSemᵀ-exists-lemma-pair₂ (interp rhs ∘ ET-to-val) id a (∃DSyn→∃DSem a rhs (fst w) , DSemᵀ-exists-lemma-identity a) 
          ih-body = ∃DSyn→∃DSem (interp rhs (ET-to-val a) , a) body (snd w)
      in DSemᵀ-exists-lemma-chain {ET Pr Γ} {σ :* ET Pr Γ} {τ} (interp body ∘ ET-to-val) (λ z → interp rhs (ET-to-val z) , z) a ih-body ih-rhs
    ∃DSyn→∃DSem {Γ} {τ} a ( prim {σ = σ} op t ) w = DSemᵀ-exists-lemma-chain {τ2 = σ} (evalprim op) (interp t ∘ ET-to-val) a (w .fst) (∃DSyn→∃DSem a t (w .snd))
    ∃DSyn→∃DSem {Γ} {τ} a ( inl t ) w = DSemᵀ-exists-lemma-chain inj₁ (interp t ∘ ET-to-val) a (DSemᵀ-inj₁ (interp t $ ET-to-val a) (zerovDense (D2τ' _ :*! D2τ' _)) .fst) (∃DSyn→∃DSem a t w) 
    ∃DSyn→∃DSem {Γ} {τ} a ( inr t ) w = DSemᵀ-exists-lemma-chain inj₂ (interp t ∘ ET-to-val) a (DSemᵀ-inj₂ (interp t $ ET-to-val a) (zerovDense (D2τ' _ :*! D2τ' _)) .fst) (∃DSyn→∃DSem a t w) 
    ∃DSyn→∃DSem {Γ} {τ} a ( case' e l r ) w
      ...
      using dg ← DSemᵀ-exists-lemma-pair₂ (interp e ∘ ET-to-val) id a (de , DSemᵀ-exists-lemma-identity a)
      with interp e (ET-to-val a) in eq1
    ∃DSyn→∃DSem {Γ} a (case' {σ = σ} {τ = τ} e l r) (we , wl) | inj₁ x
      = let dl = ∃DSyn→∃DSem (x , a) l wl
            df = Equivalence.from (DSemᵀ-exists-lemma-case-inj₁ (g a) (interp l ∘ ET-to-val) (interp r ∘ ET-to-val) x eq1) dl
            df∘g = DSemᵀ-exists-lemma-chain {τ2 = (σ :+ τ) :* ET Pr Γ} f g a df dg
      in DSemᵀ-exists-extensionality (f ∘ g) _ ext a df∘g
    ∃DSyn→∃DSem {Γ} a (case' {σ = σ} {τ = τ} e l r) (we , wr) | inj₂ x
      = let dr = ∃DSyn→∃DSem (x , a) r wr
            df = Equivalence.from (DSemᵀ-exists-lemma-case-inj₂ (g a) (interp l ∘ ET-to-val) (interp r ∘ ET-to-val) x eq1) dr
            df∘g = DSemᵀ-exists-lemma-chain {τ2 = (σ :+ τ) :* ET Pr Γ} f g a df dg
      in DSemᵀ-exists-extensionality (f ∘ g) _ ext a df∘g

    DSyn-Exists→DSem-Exists : {Γ : Env Pr} {τ : Typ Pr}  ( a : Rep (ET Pr Γ) ) → ( t : Term Pr Γ τ ) 
        → DSyn-Exists (ET-to-val a) t → (Is-just (DSemᵀ {ET Pr Γ} {τ} (interp t ∘ ET-to-val) a))
