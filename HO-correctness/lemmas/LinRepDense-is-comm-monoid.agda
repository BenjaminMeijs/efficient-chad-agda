...
LETd-to-ET : {Γ : Env Pr} → LETd (map D2τ' Γ) → LinRepDense (D2τ' (ET Pr Γ))
ET-to-LETd : {Γ : Env Pr} → LinRepDense (D2τ' (ET Pr Γ)) → LETd (map D2τ' Γ)
...
LETd-to-ET {Γ} x 
    = subst (λ x → x) (sym (ET-equiv-LETd {Γ})) x

ET-to-LETd {Γ} x 
    = subst (λ x → x) (ET-equiv-LETd {Γ}) x
